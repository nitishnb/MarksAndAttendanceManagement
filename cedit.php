...
$lab=$_POST['lab'];
$uid=$_GET['ccode'];
$ad="update course set course_name=?, total_no_of_hrs_allocated=?, credits=?, lab=? where course_code=?";
$stmt= $mysqli->prepare($ad);
$stmt->bind_param('siiss',$name,$hrs,$credits,$lab,$uid);
$stmt->execute();
// $newId = $stmtins->insert_id;
$stmt->close();
echo "<script>alert('Data updated Successfully');</script>" ;
echo "<script>location.href = 'http://localhost/project%20work/cview.php';</script>";
}?>
...
<br>&emsp;
<button class="glow-on-hover" onclick="location.href = 'cview.php';" type="button">Back</button>
<?php
$id=$_GET['ccode'];
$ret = "select * from course where course_code=?";
...
<div class="container">
 <section id="content">
   <form name="form" onsubmit="return checkCForm(this);" action="" method="post">
    <h1>Course Edit</h1><br>
	<h5>Course Code : <?php echo $row->course_code;?></h5>
<br>             <!-- Material input -->
<div class="md-form">
 <label for="form1">COURSE NAME</label>
  <input type="text" name="name" value="<?php echo $row->course_name;?>" required="required"  id="form1" class="form-control">
</div><br>
<div class="md-form">
 <label for="form1">NO. OF HOURS</label>
  <input type="text" name="hrs" value="<?php echo $row->total_no_of_hrs_allocated; ?>" required="required" id="form1" class="form-control">
</div><br>
<div class="md-form">
 <label for="form1">CREDITS</label>
  <input type="text" name="credits" value="<?php echo $row->credits; ?>" required="required" id="form1" class="form-control">
</div><br>
<div class="md-form">
 <label for="form1">LAB</label>
  <select type="text" class="form-control" id="form1" name="lab" required="required">
												<option value="<?php echo $row->lab;?>"><?php echo $row->lab;?></option>
												<optgroup label="CHOOSE ONE">
												<option value="YES">THEORY WITH LAB EXAMS-YES</option>
												 <option value="NO">ONLY THEORY EXAMS&emsp;-NO</option>
												</optgroup>
											</select>
</div><br>
 <div>
